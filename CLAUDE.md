...
Sentinel-X 是一个基于 Go 语言的微服务平台，集成了用户中心、认证授权等核心功能。
...
## 🛠️ 项目上下文 (Project Context)

### 技术栈
- **语言**: Go 1.25.0
- **Web 框架**: Gin, Echo (支持多适配器)
...
### 项目结构
- `cmd/`: 应用程序入口 (`api`, `user-center`)
- `internal/`: 私有应用代码
    - `bootstrap/`: 启动引导逻辑
    - `user-center/`: 用户中心业务逻辑 (`biz`, `data`, `handler`, `router`)
- `pkg/`: 公共库
    - `infra/`: 基础设施 (middleware, server, transport)
    - `security/`: 安全组件 (auth, jwt)
    - `utils/`: 通用工具 (errors, json, validator)
- `configs/`: 配置文件
- `staging/`: 内部依赖仓库 (`goagent`, `logger`)
...
### 🛠️ 项目特定命令 (Project Specific Commands)
使用 `Makefile` 进行日常操作：
- **构建**: `make build` (API Server), `make build-user-center` (User Center)
- **运行**: `make run-dev` (开发模式), `make run-config` (生产配置)
- **测试**: `make test` (运行所有测试), `make test-coverage` (覆盖率报告)
- **代码质量**: `make fmt` (格式化), `make lint` (静态检查)
- **依赖管理**: `make deps`, `make tidy`
...
- **示例场景**：Gin 中间件写法、Gorm 事务处理、Go Channel 模式
...
### 1. 相似实现分析
- **实现1**: internal/user-center/biz/user.go:123-156
  - 模式：[Repository模式/Service层]
  - 可复用：[UserRepo接口]
  - 需注意：[事务处理/缓存一致性]

- **实现2**: pkg/utils/errors/errno.go:78-90
  - 模式：[错误码定义]
  - 可复用：[NewError函数]
  - 需注意：[错误码唯一性]

### 2. 项目约定
- **命名约定**: [驼峰命名/接口以er结尾/包名简短]
- **文件组织**: [internal放业务/pkg放通用工具]
- **导入顺序**: [标准库 -> 第三方库 -> 项目库]
- **代码风格**: [gofumpt格式化/error处理规范]

### 3. 可复用组件清单
- `pkg/utils/validator`: 参数验证工具
- `pkg/infra/server`: 服务启动基类
- `pkg/utils/json`: JSON处理工具
- `pkg/utils/errors`: 统一错误处理

### 4. 测试策略
- **测试框架**: [Go testing/Testify]
- **测试模式**: [单元测试/Mock测试/集成测试]
- **参考文件**: internal/user-center/biz/user_test.go
- **覆盖要求**: [核心业务逻辑覆盖率 > 80%]

### 5. 依赖和集成点
- **外部依赖**: [Gin, Gorm, Redis, JWT]
- **内部依赖**: [pkg/infra, pkg/security]
- **集成方式**: [依赖注入/接口调用]
- **配置来源**: [configs/user-center.yaml]
...
### 7. 关键风险点
- **并发问题**: [Goroutine泄漏/Race Condition]
- **边界条件**: [空指针/零值处理]
- **性能瓶颈**: [数据库慢查询/锁竞争]
- **安全考虑**: [SQL注入/XSS/越权访问]
##### ```
