...
if [ ! -d "${SOURCE_DIR}" ]; then
    sentinel::log::error "Error: Source directory ${SOURCE_DIR} does not exist."
    exit 1
fi

sentinel::log::info "===> Publishing ${STAGING_PATH} to ${REMOTE_REPO} (${BRANCH})..."

# 1. Clone the remote repository to a temporary location
sentinel::log::info "-> Cloning remote repository..."
if ! git -c http.proxy= -c https.proxy= clone --depth 1 --branch "${BRANCH}" "${REMOTE_REPO}" "${TMP_DIR}"; then
     sentinel::log::error "Error: Failed to clone ${REMOTE_REPO}."
     rm -rf "${TMP_DIR}"
     exit 1
fi

# 2. Sync content
sentinel::log::info "-> Syncing content..."
...
if [[ -z $(git status --porcelain) ]]; then
    sentinel::log::info "-> No changes detected. Remote is up to date."
else
    git add -A
...
Date: $(date -u +"%Y-%m-%dT%H:%M:%SZ")"
    ...
    sentinel::log::info "-> Pushing changes to remote..."
    git push origin "${BRANCH}"

    sentinel::log::info "===> Successfully published to ${REMOTE_REPO}"
fi

# Cleanup
rm -rf "${TMP_DIR}"
sentinel::log::info "Done."
