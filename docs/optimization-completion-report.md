...
**完成内容**：
- 新增 `internal/rag/metrics/metrics.go`（RAG 业务指标收集器）
- 集成到 `internal/rag/biz/service.go`（记录查询、缓存、检索、LLM 指标）
- 更新 `internal/rag/handler/rag.go`（添加 Metrics 导出端点）
- 更新 `internal/rag/router/router.go`（注册 /metrics 路由）
- 新增 `docs/monitoring/grafana-dashboard.json`（完整 Grafana 仪表盘配置）
- 新增 `docs/monitoring/alerting-rules.md`（Prometheus 告警规则和 Runbook）
- 新增 `docs/monitoring/setup-guide.md`（监控系统搭建指南）

**核心功能**：

1. **业务指标收集（RAGMetrics）**
   - 查询指标：总次数、缓存命中/未命中、错误率、缓存命中率
   - 检索指标：总次数、耗时、错误率
   - LLM 指标：调用次数、耗时、错误率、重试次数、Token 使用量
   - 熔断器指标：状态（Closed/Open/Half-Open）、打开次数
   - 索引指标：文档数、分块数、错误次数
   - 运行时指标：服务运行时间
   - 线程安全：使用 atomic 操作保证并发安全

2. **Prometheus 集成**
   - `/v1/rag/metrics` 端点导出 Prometheus 格式指标
   - 支持自定义 namespace 和 subsystem
   - 标准 HELP 和 TYPE 注释
...
4. **告警规则体系**
   - 查询相关：高错误率、QPS 异常
   - 缓存相关：命中率过低
   - LLM 相关：高错误率、高延迟、高重试、Token 使用量激增
   - 熔断器相关：打开告警、频繁抖动
   - 检索相关：高错误率、高延迟
   - 索引相关：错误率过高
   - 告警级别：Critical / Warning / Info
   - 告警抑制：避免告警风暴
   - 告警路由：Slack / PagerDuty / 邮件

5. **监控文档**
   - Docker Compose 一键部署
   - Prometheus / Grafana / Alertmanager 配置示例
   - PromQL 查询示例
   - 生产环境最佳实践
   - 故障排查指南
...
本次优化工作全部完成，显著提升了 Sentinel-X 项目的代码质量、安全性、可靠性和可观测性：
...
项目已进入更加稳定、可维护、可观测的状态。建议继续完成测试覆盖率提升和性能优化工作。
