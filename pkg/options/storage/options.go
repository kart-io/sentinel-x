...
func (o *CommonOptions) Validate() error {
	if o.MaxRetries < -1 {
		return fmt.Errorf("MaxRetries must be >= -1")
	}
	...
	if o.PoolSize < 0 {
		return fmt.Errorf("PoolSize must be non-negative")
	}

	if o.MinIdleConns < 0 {
		return fmt.Errorf("MinIdleConns must be non-negative")
	}

	if o.MinIdleConns > o.PoolSize && o.PoolSize > 0 {
		return fmt.Errorf("MinIdleConns cannot exceed PoolSize")
	}
