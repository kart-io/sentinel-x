...
func WithFilter(filter map[interface{}]interface{}) Option {
	return func(whr *Options) {
		whr.Filters = filter
	}
}
...
func (whr *Options) F(kvs ...interface{}) *Options {
	if len(kvs)%2 != 0 {
...
func T(ctx context.Context) *Options {
	if registeredTenant.ValueFunc == nil {
		return NewWhere()
	}
