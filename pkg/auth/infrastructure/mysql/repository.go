...
func (r *Repository) LoadPolicies(ctx context.Context) ([]*auth.Policy, error) {
	...
	policies := make([]*auth.Policy, len(rules))
	for i, rule := range rules {
		policies[i] = &auth.Policy{
			PType: rule.PType,
			V0:    rule.V0,
			V1:    rule.V1,
			V2:    rule.V2,
...
func (r *Repository) AddPolicy(ctx context.Context, p *auth.Policy) error {
	rule := CasbinRule{
		PType: p.PType,
		V0:    p.V0,
		V1:    p.V1,
...
func (r *Repository) RemovePolicy(ctx context.Context, p *auth.Policy) error {
	rule := CasbinRule{
		PType: p.PType,
		V0:    p.V0,
		V1:    p.V1,
