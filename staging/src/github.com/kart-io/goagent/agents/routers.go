...
func (r *LLMRouter) Route(ctx context.Context, task Task, agents map[string]core.Agent) (string, error) {
	...
	if err != nil {
		return "", agentErrors.Wrap(err, agentErrors.CodeExternalService, "LLM routing failed").
	...
	if _, exists := agents[agentName]; !exists {
		...
		return "", agentErrors.New(agentErrors.CodeNotFound, "no suitable agent found").
			WithComponent("agent_router").
			WithOperation("Route")
	}
...
func (r *RuleBasedRouter) Route(ctx context.Context, task Task, agents map[string]core.Agent) (string, error) {
	...
	return "", agentErrors.New(agentErrors.CodeNotFound, "no agents available").
		WithComponent("agent_router").
		WithOperation("Route")
}
...
func (r *LoadBalancingRouter) Route(ctx context.Context, task Task, agents map[string]core.Agent) (string, error) {
	...
	return "", agentErrors.New(agentErrors.CodeResourceLimit, "all agents at maximum capacity").
		WithComponent("agent_router").
		WithOperation("Route")
}
