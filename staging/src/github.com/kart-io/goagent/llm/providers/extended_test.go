...
func TestDeepSeekProvider_CompleteWithAllFields(t *testing.T) {
	...
	provider, err := NewDeepSeekWithOptions(agentllm.WithAPIKey("test-key"), agentllm.WithBaseURL(mockServer.URL), agentllm.WithModel("deepseek-chat"), agentllm.WithMaxTokens(1000), agentllm.WithTemperature(0.8), agentllm.WithTimeout(30 * time.Second))
	require.NoError(t, err)
...
func TestDeepSeekStreamingProvider_StreamWithMetadata_ErrorHandling(t *testing.T) {
	...
	provider, err := NewDeepSeekStreamingWithOptions(agentllm.WithAPIKey("test-key"), agentllm.WithBaseURL(mockServer.URL), agentllm.WithTimeout(30 * time.Second))
	require.NoError(t, err)
...
func TestDeepSeekProvider_CompleteWithModelOverride(t *testing.T) {
	...
	provider, err := NewDeepSeekWithOptions(agentllm.WithAPIKey("test-key"), agentllm.WithBaseURL(mockServer.URL), agentllm.WithModel("deepseek-chat"), agentllm.WithTimeout(30 * time.Second))
	require.NoError(t, err)
